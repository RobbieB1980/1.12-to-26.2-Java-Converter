...
namespace RB.JavaConverter112.Setup;

public sealed class SetupForm : Form
{
    private const string AppDisplayName = "RB 1.12 to 26.2 Java Converter";
    private const string ExeName = "RB-112-to-262-Java-Converter.exe";
    private const string UninstallRegKey = @"Software\Microsoft\Windows\CurrentVersion\Uninstall\RB-112-to-262-Java-Converter";
    private const string AppPathsKey = @"Software\Microsoft\Windows\CurrentVersion\App Paths\RB-112-to-262-Java-Converter.exe";
    private const string DesktopLnk = "RB 1.12 to 26.2 Java Converter.lnk";
    private const string StartMenuFolder = "RB 1.12 to 26.2 Java Converter";
    ...
    public SetupForm()
    {
        ...
        var sub = new Label
        {
            Text = "Installs a portable tool for Forge 1.12.2 → NeoForge 26.2 conversion (experimental).\nUninstall is available here, from Start Menu, or Apps & features.",
    ...
    {
        var version = Assembly.GetExecutingAssembly().GetName().Version?.ToString(3) ?? "0.9.2";
...
$Product = '" + AppDisplayName.Replace("'", "''") + @"'
$ExeBase = '" + Path.GetFileNameWithoutExtension(ExeName).Replace("'", "''") + @"'
$DesktopLnk = Join-Path ([Environment]::GetFolderPath('Desktop')) '" + DesktopLnk.Replace("'", "''") + @"'
$StartMenu = Join-Path ([Environment]::GetFolderPath('StartMenu')) 'Programs\\" + StartMenuFolder.Replace("'", "''") + @"'
$UninstallKey = 'HKCU:\\" + UninstallRegKey.Replace("'", "''") + @"'
$AppPaths = 'HKCU:\\" + AppPathsKey.Replace("'", "''") + @"'
...
";
    ...
    private static string? FindPayloadZip()
    {
        ...
        var names = new[]
        {
            ...
            "RB-112-to-262-Java-Converter-Portable.zip",
            "payload.zip"
        };
        ...
        {
            var temp = Path.Combine(Path.GetTempPath(), "RB-112-payload-" + Guid.NewGuid().ToString("N") + ".zip");
